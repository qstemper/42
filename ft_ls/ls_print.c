...
/*   Updated: 2016/10/06 11:13:57 by qstemper         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void			p_long(void *p, size_t size)
{
	...
	if (!e->handle && !((env()->o & O_HIDE) == O_HIDE))
		return ;
	env()->i++;
	ft_printf("%c%s%s%s%c %*d % -*s % -*s% *d %.3s %2.s %s %s", \
			fmt_file_mode(e), fmt_owner_perm(e, buff[0]), \
			fmt_group_perm(e, buff[1]), fmt_other_perm(e, buff[2]), \
			fmt_attribute(e), env()->nlinkpad, fmt_nblink(e), \
			env()->ownerlen + 1, fmt_owner(e), env()->grplen + 1, \
			fmt_group(e), env()->sizepad + 1, fmt_size(e), \
			fmt_month(e, buff[3]), fmt_day(e, buff[4]), \
